...
public class EngineFactory {
	...
	private static final MyPoolableObjectFactory factory = new MyPoolableObjectFactory();
...
//		PooledObjectFactory<Resource> factory = new MyPoolableObjectFactory();
		...
		// 创建对象池
		pool = new GenericObjectPool<Resource>(factory, poolConfig);
        for (int i = 0; i < 12; i++) {
	...
	public GenericObjectPool<Resource> getObjectPool() {
		if (pool == null) {
			synchronized (lock) {
				if (pool == null) {
					init();
